...
import { S3MixIn, Service, Webda } from "../services/s3-mixin";
...
export default class CloudTrailSetup extends S3MixIn(Configurer) {
  ...
  async setupKMSKey(kms, keyName, accountId: string, principals: string[]) {
    ...
    if (currentAlias) {
      return (
        currentAlias.AliasArn.substr(0, currentAlias.AliasArn.lastIndexOf(":")) + ":key/" + currentAlias.TargetKeyId
      );
    }
      ...
      if (!notification) {
        if (!queue.Policy) {
          let policy = {
            Version: "2012-10-17",
            Id:
              "arn:aws:sqs:" + mainRegion + ":" + account.Id + ":" + this._params.cloudTrailQueue + "/SQSDefaultPolicy",
            Statement: [
              {
                Sid: "Ivoryshield",
                Effect: "Allow",
  ...
  static getModda() {
    return {
      ...
      description:
        "Enable CloudTrail on every accounts, create S3 bucket to store the trails, create a backup S3 bucket for multi-region disaster recovery, and a KMS key to encrypt the trails",
      webcomponents: [],
