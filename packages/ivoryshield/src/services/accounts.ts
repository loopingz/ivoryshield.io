...
export default class AccountsService extends IvoryShieldService {
  ...
  normalizeParams() {
    this.staticConfiguration = this.getParameters().accounts !== undefined;
    if (this.staticConfiguration) {
      this.accounts = this.getParameters().accounts;
      this.sortAccounts();
    }
  }
  ...
  async refreshOrganization() {
    let aws = this._aws;
    if (this.getParameters().organizationAccountId) {
      aws = await this._getAWSForAccount(this.getParameters().organizationAccountId);
    }
    ...
    try {
      ...
      await Promise.all(
        this.accounts.map(async (act, index) => {
          let tok = await this._sts
            .assumeRole({
              DurationSeconds: 3600,
              ExternalId: this.getParameters().externalId,
              RoleArn: "arn:aws:iam::" + act.Id + ":role/" + this.getParameters()["role"],
  ...
  getMainAccountId() {
    return this.getParameters().mainAccount;
  }

  async getMainAccountAWS(region: string = "us-east-1") {
    return this._getAWSForAccount(this.getParameters().mainAccount, region);
  }

  isMainAccount(account) {
    return account === this.getParameters().mainAccount;
  }
...
export { AccountsService, AccountMap };
