...
module.exports = class CronCheckerService extends AWSService(Service) {
  init(config) {
    super.init(config);
    ...
    this._params.configurers.forEach( (configurer) => {
      let service = this.getService(configurer);
      ...
      if (service.isGlobal()) {
        ...
        this._globalConfigurers.push(this.getService(configurer));
      } else {
        console.log('Adding configurer: ', configurer);
        this._configurers.push(this.getService(configurer));
      }
    });
  }
  ...
  _handleResource(aws, resources, type, account) {
  ...
  checkSecurityGroups(aws, account) {
    let ec2 = new aws.EC2();
  ...
  checkNetworkInterfaces(aws, account) {
    let ec2 = new aws.EC2();
  ...
  checkLoadBalancers(aws, account) {
    let ec2 = new aws.EC2();
  ...
  checkCustomerGateways(aws, account) {
    let ec2 = new aws.EC2();
  ...
  checkInternetGateways(aws, account) {
    let ec2 = new aws.EC2();
  ...
  checkIAMRoles(aws, account) {
    let iam = new aws.IAM();
    return s3.listBuckets().promise().then( (res) => {
      let promise = Promise.resolve();
      res.Buckets.forEach((inst) => {
        promise = promise.then( () => {
          return this._handleResource(aws, inst, 'Role', account);
        });
      });
      return promise;
    });
  ...
  checkRds(aws, account) {
    let ec2 = new aws.EC2();
    return ec2.describeSnapshots().promise().then( (res) => {
      console.log(res);
      return Promise.resolve();
      let promise = Promise.resolve();
      res.Volumes.forEach((inst) => {
        promise = promise.then( () => {
          return this._handleResource(aws, inst, 'Rds', account);
        });
      });
      return promise;
    });
  ...
  check(aws) {
    ...
    return ec2.describeSnapshots().promise().then( (res) => {
      ...
      res.Volumes.forEach((inst) => {
        promise = promise.then( () => {
          return this._handleResource(aws, inst, '', account);
        });
      });
      return promise;
    });
